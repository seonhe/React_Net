...
class GeneralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, conv, kernel_size=3, padding=1, stride=1, group=1):
        super().__init__()
...
class depthwise_separable_conv(nn.Module):
    def __init__(self, nin, nout, kernel_size=3, kernels_per_layer=1, stride=1, conv):
      super(depthwise_separable_conv, self).__init__()
      self.depthwise = GeneralConv2d(in_channels=nin, out_channels=nin * kernels_per_layer, kernel_size=kernel_size, padding=1, groups=nin, stride=stride, conv=conv)
      self.pointwise = GeneralConv2d(in_channels=nin * kernels_per_layer, out_channels=nout, kernel_size=1, stride=1, padding=0, conv=conv)
